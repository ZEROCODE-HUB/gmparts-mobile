...
Future<String> actualizarStockDesdeListas(
  List<FacturasArticulosStruct>? listaAntigua,
  List<FacturasArticulosStruct>? listaNueva,
) async {
  try {
    // Si no llega listaNueva, no se puede actualizar nada
    if (listaNueva == null) {
      return 'Error: La lista nueva es obligatoria.';
    }

    // Caso 1: primera vez, solo hay listaNueva
    if (listaAntigua == null) {
      ...
      for (final articulo in listaNueva) {
        ...
        if (ref != null && ref.id != null) {
          final res = await _actualizarStock(ref.id, cantidad);
          if (!res.success) return res.message;
          nuevos++;
        }
      }
      return 'Stock inicial agregado: $nuevos artículos nuevos.';
    }

    // Caso 2: flujo normal con ambas listas

    // Debug: referencias iguales (seguridad extra)
    for (int i = 0; i < listaNueva.length; i++) {
      if (i < listaAntigua.length &&
          identical(listaNueva[i], listaAntigua[i])) {
    ...
    for (final id in antiguaMap.keys) {
      if (!nuevaMap.containsKey(id)) {
        ...
        final res = await _actualizarStock(id, -cantidad);
        if (!res.success) return res.message;
        eliminados++;
      }
    }
    ...
    for (final id in nuevaMap.keys) {
      ...
      if (antiguaMap.containsKey(id)) {
        ...
        if (nuevaCantidad != viejaCantidad) {
          ...
          final res = await _actualizarStock(id, diferencia);
          if (!res.success) return res.message;
          modificados++;
        }
      } else {
    ...
    return 'Actualización exitosa: $nuevos nuevos, $modificados modificados, $eliminados eliminados.';
  } catch (e) {
...
Future<StockResult> _actualizarStock(String docId, int cambio) async {
  try {
    ...
    if (!snapshot.exists) {
      return StockResult(
          false, 'El artículo con ID $docId no existe en la base de datos.');
    }
