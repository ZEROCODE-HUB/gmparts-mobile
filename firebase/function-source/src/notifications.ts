 ...
 */
async function collectTokens(events: RecepcionEvent[]): Promise<string[]> {
  const roles = new Set<string>()
  for (const event of events) {
    for (const role of EVENT_ROLES[event]) {
      roles.add(role)
    }
  }
 ...
 */
export const onRecepcionPush = functions.firestore
  .document('recepciones/{recepcionId}')
  .onWrite(async (change, context) => {
    const before: SnapshotData = change.before?.data() ?? null
    const after: SnapshotData = change.after?.data() ?? null
    ...
    const recepcionId = context.params.recepcionId as string
    const record: SnapshotData = after ?? before

    const tokens = await collectTokens(events)
    if (tokens.length === 0) {
      functions.logger.log(
        'onRecepcionPush: sin tokens FCM registrados para los eventos',
        { events, recepcionId }
      )
      return
    }
    ...
    for (const event of events) {
      const payload = buildEventPayload(event, record, recepcionId)
